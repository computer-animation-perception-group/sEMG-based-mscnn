...
class CrossValEvaluation(Evaluation):
    ...
    def topk_accuracy_curves(self, exps, folds, windows, feature_name, window, num_semg_row, num_semg_col, topk, balance=False):
        acc = []
        for exp in exps:
            for fold in folds:
                acc.append(self.topk_accuracy_curve(
                    Mod=exp.Mod,
                    dataset=exp.dataset,
                    fold=int(fold),
                    windows=windows,
    ...
    def topk_accuracy_curve(self, Mod, dataset, fold, windows, feature_name, window, num_semg_row, num_semg_col,
                            topk,
                            dataset_args=None,
                            balance=False):
        get_crossval_val = self.get_crossval_val_func(dataset)
        pred, true, segment = _crossval_predict(
            ...
            num_semg_col = num_semg_col,
            dataset_args=dataset_args)
        pred_label = np.argsort(pred, axis=1)
        num_classes = pred_label.shape[1]
        topk = min(num_classes, topk)        
        pred = np.ones(true.shape)*-1
                
        print 'Fold:%d' % fold
        for i in range(pred_label.shape[0]):
            for j in range(topk):
                if pred_label[i, num_classes - 1 - j] == true[i]:
                    pred[i] = true[i]
                    break
