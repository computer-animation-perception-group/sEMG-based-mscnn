...
class CapgMyoPieceSigImgMultiStreamData(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CapgMyoPieceSigImgMultiStreamData, self).__init__(*args, **kargs)

        self.num_channel = []
        self.num_stripe = len(genIndex(self.num_semg_col))
        self.shapes = []        
...
#        print data.shape
        res = []
        index = genIndex(self.num_semg_col)
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CapgMyoPieceSigImgMultiStreamData, self).reset()
