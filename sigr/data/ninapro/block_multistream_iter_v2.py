...
class BlockMultiStreamData_v2(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(BlockMultiStreamData_v2, self).__init__(*args, **kargs)
...
#        self.ch_win_num = 2
#        
#        self.total_ch = self.num_semg_col*self.num_semg_row -2 - self.ch_win_num + 1
#
#        self.num_channel = []
#        self.shapes = []        
#        for i in range(self.total_ch):
#            self.num_channel.append(1)
#            self.shapes.append(self.ch_win_num)
#            
#        self.num_channel.append(1)
...
#         res = [(k, tuple([self.batch_size, self.num_channel] + list([self.window, self.shape_2]))) for k, v in self.data]
         ...
         res = [('stream%d_' % i + self.data[0][0], tuple([self.batch_size, ch] + list([self.shapes[i], self.window])))
...
#              ch_data = ch_data.reshape(ch_data.shape[0], 1, ch_data.shape[2], -1)
#              res.append(ch_data)         
...
#        assert self.window == 1    
    ...
    def reset(self):
        self._reset()
        super(BlockMultiStreamData_v2, self).reset()
