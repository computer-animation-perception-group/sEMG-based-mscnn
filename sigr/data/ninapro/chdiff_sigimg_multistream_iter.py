...
class ChDiffSigImgMultiStreamData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(ChDiffSigImgMultiStreamData, self).__init__(*args, **kargs)
        ...
        shape_1 = int(np.math.factorial(self.num_semg_col*self.num_semg_row)/(np.math.factorial(self.num_semg_col*self.num_semg_row-2)*2))
        shape_2 = len(genIndex(self.num_semg_col*self.num_semg_row))-1
...
#         res = [(k, tuple([self.batch_size, self.num_channel] + list([self.window, self.shape_2]))) for k, v in self.data]
         ...
         res = [('stream%d_' % i + self.data[0][0], tuple([self.batch_size, ch] + list([self.window, self.shapes[i]])))
    ...
    def _get_chdiffimg(self, data):
        
        from ... import Context
        import joblib as jb
        res = []
         
        for amp in Context.parallel(jb.delayed(_get_chdiffimg_aux)(sample) for sample in data):
...
#             print res[0].shape
             res_1 = [self._get_chdiffimg(a.asnumpy() if isinstance(a, mx.nd.NDArray) else a) for a in res]
             ...
             res_2 = [a.reshape(a.shape[0], 1, a.shape[1], -1) for a in res_2]               
...
#             assert (res[0].shape[3] == self.shape_1)
    ...
    def reset(self):
        self._reset()
        super(ChDiffSigImgMultiStreamData, self).reset()
...
def _get_chdiffimg_aux(data):    
            
    return np.transpose(get_chdiff_img(data.T))
#    res = np.transpose(np.concatenate((get_chdiff_img(data.T), get_sig_img(data.T)),  axis=0))
#    print res.shape
...
#     print data.shape
     ...
     for i in range(data.shape[1]):
        ...
        for j in range(ch_num):
            for k in range(j+1, ch_num):
                frame_res.append(frame[k]-frame[j])
        frame_res = np.array(frame_res)        
