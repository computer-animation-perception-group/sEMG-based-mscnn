...
class FeatureMapData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(FeatureMapData, self).__init__(*args, **kargs)
        
        self.num_channel = self._get_feature_dim()
        self.num_data = len(self._index)
        self.data_orig = self.data
        self.reset()
        for feature_n in self.feature_list:
           print feature_n
        print ("total feature dims =  %d" % (self.num_channel)) 
...
#        print ("total feature dims =  %d" % (feature.shape[1])) 
    ...
    def _get_feature(self, data):
        ...
        assert(res.shape[1] == self.num_channel)
...
#        res = res.reshape(res.shape[0], res_axis_1, res_axis_2)
        ...
        res = res.reshape(res.shape[0], res.shape[1], self.num_semg_row, self.num_semg_col)      
 ...
 #            res = [a.reshape(a.shape[0], a.shape[1], self.num_semg_row, self.num_semg_col) for a in res]
    ...
    def reset(self):
        self._reset()
        super(FeatureMapData, self).reset()
