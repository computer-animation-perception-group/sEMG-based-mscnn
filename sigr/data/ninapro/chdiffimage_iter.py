...
class ChDiffImageData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(ChDiffImageData, self).__init__(*args, **kargs)

       
        self.num_data = len(self._index)
        self.data_orig = self.data
        self.reset()
        self.shape_1 =  int(np.math.factorial(self.num_semg_col*self.num_semg_row)/(np.math.factorial(self.num_semg_col*self.num_semg_row-2)*2))
        self.shape_2 = self.shape_1 + (len(genIndex(self.num_semg_col*self.num_semg_row))-1)
        print self.shape_2
...
#             print res[0].shape
             res = [self._get_chdiffimg(a.asnumpy() if isinstance(a, mx.nd.NDArray) else a) for a in res]
...
#             assert (res[0].shape[3] == self.shape_1)
    ...
    def reset(self):
        self._reset()
        super(ChDiffImageData, self).reset()
