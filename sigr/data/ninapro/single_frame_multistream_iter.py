...
class SingleFrameMultiStreamData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(SingleFrameMultiStreamData, self).__init__(*args, **kargs)
...
#         res = [(k, tuple([self.batch_size, self.num_channel] + list([self.window, self.shape_2]))) for k, v in self.data]
         ...
         res = [('stream%d_' % i + self.data[0][0], tuple([self.batch_size, ch] + list([self.num_semg_row, self.num_semg_col])))
...
#        print data.shape
        ...
        for i in range(data.shape[1]):
              frame_data = data[:,i,:]
              frame_data = frame_data.reshape(frame_data.shape[0], 1, 1, -1)
...
#             print res[0].shape    
    ...
    def reset(self):
        self._reset()
        super(SingleFrameMultiStreamData, self).reset()
