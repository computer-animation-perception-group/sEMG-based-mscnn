...
class ChDiffSigImageData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        super(ChDiffSigImageData, self).__init__(*args, **kargs)
...
#         res = [(k, tuple([self.batch_size, self.num_channel] + list(v.shape[2:]))) for k, v in self.data]
         res = [(k, tuple([self.batch_size, self.num_channel] + list([self.window, len(genIndex(self.num_semg_col*self.num_semg_row-1))-1]))) for k, v in self.data]
         return res
    ...
    def _get_chdiffsigimg(self, data):
        
        from ... import Context
        import joblib as jb
        res = []
         
        for amp in Context.parallel(jb.delayed(_get_chdiffsigimg_aux)(sample) for sample in data):
...
#             print res[0].shape
             res = [self._get_chdiffsigimg(a.asnumpy() if isinstance(a, mx.nd.NDArray) else a) for a in res]
...
#             assert (res[0].shape[3] == self.shape_1)
    ...
    def reset(self):
        self._reset()
        super(ChDiffSigImageData, self).reset()
...
def _get_chdiffsigimg_aux(data):    
          
    return np.transpose(get_chdiffsig_img(data.T))   
...
def get_chdiffsig_img(data):   
#     print data.shape
     return get_sig_img(get_chdiff_img(data))     
...
def get_chdiff_img(data):
     ...
     for i in range(data.shape[1]):
        ...
        for j in range(1,ch_num):
            frame_res.append(frame[j]-frame[0])
