...
class FeatureImageData(mx.io.NDArrayIter):
    ...
    def __init__(self, *args, **kargs):
        ...
        self.num_channel = 1
        super(FeatureImageData, self).__init__(*args, **kargs)
        
        self.feature_dim = self._get_feature_dim()
        self.num_data = len(self._index)
        self.data_orig = self.data
        self.reset()
        for feature_n in self.feature_list:
           print feature_n
        print ("total feature dims =  %d" % (self.feature_dim)) 
    ...
    def provide_data(self):
         ...
         res = [(k, tuple([self.batch_size, self.num_channel] + list([self.feature_dim, self.num_semg_col*self.num_semg_row]))) for k, v in self.data]
         return res
     
    def _get_feature_dim(self):
        data_slice = self.data[0][1][0:self.window,]
        data_slice = data_slice.reshape(data_slice.shape[0],-1)
#        print data_slice.shape
        feature = get_feature(data_slice.T, self.feature_list)      
#        print ("total feature dims =  %d" % (feature.shape[1])) 
    ...
    def _get_feature(self, data):
        ...
        for amp in Context.parallel(jb.delayed(_get_feature_aux)(sample, self.feature_list) for sample in data):
            res.append(amp[np.newaxis, ...])
        res = np.concatenate(res, axis=0)
        assert(res.shape[1] == self.feature_dim)
        
#        # test feature  normalization in provide data
#        res_axis_1 = res.shape[1]
#        res_axis_2 = res.shape[2]
#        res = res.reshape(res.shape[0], -1)        
#        res_std = res.std(axis=0, ddof=1)
#        res_std[np.where(res_std == 0)[0]] = 1e-5
#        res = ((res - res.mean(axis=0))/res_std)*2.0
#        res = res.reshape(res.shape[0], res_axis_1, res_axis_2)
        
        res = res.reshape(res.shape[0], self.num_channel, self.feature_dim, self.num_semg_row*self.num_semg_col)      
    ...
    def _getdata(self, data_source):
        ...
        if data_source is self.data:       
             ...
             res = [self._get_feature(a.asnumpy() if isinstance(a, mx.nd.NDArray) else a) for a in res]
 #            res = [a.reshape(a.shape[0], a.shape[1], self.num_semg_row, self.num_semg_col) for a in res]
    ...
    def reset(self):
        self._reset()
        super(FeatureImageData, self).reset()
