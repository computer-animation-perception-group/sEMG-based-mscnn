...
class Dataset(Base):
    ...
    num_semg_row = feature_extraction_winlength
    num_semg_col = 50
...
class GetTrial(object):
    ...
    def __init__(self, gestures, trials, feature_name, preprocess=None):
        self.memo = {}
    ...
    def get_path(self, root, combo):
        return os.path.join(root,
                            '{0.subject:03d}',
                            '{0.gesture:03d}',
                            '{0.subject:03d}_{0.gesture:03d}_{0.trial:03d}_{1}.mat'
                           ).format(combo, self.feature_name)
...
@utils.cached
def _get_data(paths):
    return [_get_data_aux(path) for path in paths]


def _get_data_aux(path):
    
    logger.debug('Load {}', path)
    mat = sio.loadmat(path)
    data = mat['data'].astype(np.float32)
    gesture = np.repeat(label_to_gesture(np.asscalar(mat['label'].astype(np.int))), data.shape[0])
    subject = np.repeat(np.asscalar(mat['subject'].astype(np.int)), data.shape[0])
