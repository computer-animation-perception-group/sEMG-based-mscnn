...
class CSLPieceWiseMultiStreamData_V2(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CSLPieceWiseMultiStreamData_V2, self).__init__(*args, **kargs)

        self.piece_width = 2
           
        self.num_channel = []
        self.num_stripe =  self.num_semg_row - self.piece_width + 1
        self.shapes1 = []        
        self.shapes2 = []             
...
#              print ch_data.shape
              ch_data = ch_data.reshape(ch_data.shape[0], ch_data.shape[1], end-start, -1)
#              ch_data = ch_data.reshape(ch_data.shape[0], ch_data.shape[1], self.piece_width, -1)
              res.append(ch_data) 
              
             
#        for i in range(self.num_stripe -1 ):      
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CSLPieceWiseMultiStreamData_V2, self).reset()
