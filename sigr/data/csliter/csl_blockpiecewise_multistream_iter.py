...
class CSLBlockPieceWiseMultiStreamData(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CSLBlockPieceWiseMultiStreamData, self).__init__(*args, **kargs)
...
#        self.block_width = int(num_block_stripes*2)
        ...
        self.shapes1 = []        
        self.shapes2 = [] 
        self.total_ch_1 = self.num_semg_col - self.ch_win_num + 1            
        for i in range(self.total_ch_1):
...
#            self.shapes2.append(self.num_semg_col)
        ...
        self.total_ch_2 = int(self.num_semg_row/self.piece_width)         
        for i in range(self.total_ch_2):
            self.num_channel.append(self.window)
...
#              print ch_data.shape
        ...
        for i in range(self.total_ch_2):
              ...
              ch_data = data[:,:,start:end,:]
#              print ch_data.shape
              ch_data = ch_data.reshape(ch_data.shape[0], ch_data.shape[1], self.piece_width, -1)
              res.append(ch_data) 
...
#             kk = kk.reshape(kk.shape[0], shape_1, shape_2, shape_3)             
    ...
    def reset(self):
        self._reset()
        super(CSLBlockPieceWiseMultiStreamData, self).reset()
