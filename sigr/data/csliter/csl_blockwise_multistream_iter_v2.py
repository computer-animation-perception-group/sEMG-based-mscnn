...
class CSLBlockWiseMultiStreamData_V2(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CSLBlockWiseMultiStreamData_V2, self).__init__(*args, **kargs)
...
#        self.block_width = int(num_block_stripes*2)
        ...
        self.total_ch = int ((self.num_semg_col + 2) / self.ch_win_num)
...
#            self.shapes2.append(1)
        ...
        self.num_channel.append(self.window)
        self.shapes1.append(self.num_semg_row*3)
        self.shapes2.append(1) 
...
#        first_row =   data[:,:,:,0]
#        first_row = first_row.reshape(first_row.shape[0], first_row.shape[1], first_row.shape[2], 1)
#        
#        last_row =   data[:,:,:,0]
#        last_row = last_row.reshape(last_row.shape[0], last_row.shape[1], last_row.shape[2], 1)
#        
#        data = np.concatenate((first_row, data, last_row), axis=3)
#        
##        print data.shape
#        
#        for i in range(self.total_ch):
#              start = i*self.ch_win_num
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CSLBlockWiseMultiStreamData_V2, self).reset()
