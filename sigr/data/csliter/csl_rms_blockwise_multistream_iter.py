...
#            self.shapes1.append(4)
#            self.shapes2.append(6)
...
#         return res
    ...
    def _get_rms(self, data):
        for sample in data:
            sample = np.sum(np.square(kk) for kk in sample, axis=0)
...
#        print data.shape
        ...
        first_row = data[:,:,:,0]
        first_row = first_row.reshape(first_row.shape[0], first_row.shape[1], -1, 1)

        last_row = data[:,:,:,-1]
        last_row = last_row.reshape(last_row.shape[0], last_row.shape[1], -1, 1)    
...
#        first_row = first_row.reshape(first_row.shape[0], first_row.shape[1], -1, 1)    
#        
#        data = np.concatenate((first_row, data), axis=3)        
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
             ...
             assert (len(res) == 1)   
             kk = res[0]
             kk = kk.asnumpy() if isinstance(kk, mx.nd.NDArray) else kk             
             kk = self._get_rms(kk)
             res = self._get_ch(kk)                        
