...
class CSLBlockWiseDiffMultiStreamData(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CSLBlockWiseDiffMultiStreamData, self).__init__(*args, **kargs)
...
#        self.block_width = int(num_block_stripes*2)
        ...
        self.total_ch = self.num_semg_col - self.ch_win_num + 1 -1
...
#        print data.shape
        ...
        for i in range(self.total_ch):
              ...
              ch_data_1 = data[:,:,:,start:end]
              ch_data_1 = ch_data_1.reshape(ch_data_1.shape[0], ch_data_1.shape[1], -1, end-start)
              
              start = i+1
              end = start+self.ch_win_num
              ch_data_2 = data[:,:,:,start:end]
              ch_data_2 = ch_data_2.reshape(ch_data_2.shape[0], ch_data_2.shape[1], -1, end-start) 
               
#              print ch_data.shape
              ch_diff = ch_data_2 - ch_data_1
              ch_diff = ch_diff*10
              res.append(ch_diff)  
        return res      
...
#             print len(self.num_channel)                       
    ...
    def reset(self):
        self._reset()
        super(CSLBlockWiseDiffMultiStreamData, self).reset()
