...
class CapgMyoTwoAxisPieceWiseMultiStreamData(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CapgMyoTwoAxisPieceWiseMultiStreamData, self).__init__(*args, **kargs)

        self.num_channel = []
        self.num_stripe = self.num_semg_row
        self.shapes = []        
        for i in range(self.num_semg_row):
            self.num_channel.append(self.window)
            self.shapes.append(self.num_semg_col)
            
        for i in range(self.num_semg_col):
            self.num_channel.append(self.window)
            self.shapes.append(self.num_semg_row)    
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CapgMyoTwoAxisPieceWiseMultiStreamData, self).reset()
