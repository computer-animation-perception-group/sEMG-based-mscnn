...
class CapgMyoPiecePlusRawImgMultiStreamData(mx.io.NDArrayIter):
...
#        self.num_channel = 1    
        ...
        super(CapgMyoPiecePlusRawImgMultiStreamData, self).__init__(*args, **kargs)

        self.num_channel = []
        self.num_stripe = int(self.num_semg_col)
        self.shapes1 = []        
        self.shapes2 = []        
        for i in range(self.num_stripe):
            self.num_channel.append(self.window)
            self.shapes1.append(2)
            self.shapes2.append(int(self.num_semg_row/2))
...
#              print ch_data.shape
#              ch_data = ch_data.reshape(ch_data.shape[0], ch_data.shape[1], 2, -1)
              res.append(ch_data)  
...
#             res = [a.reshape(a.shape[0], a.shape[1], -1) for a in res]  
    ...
    def reset(self):
        self._reset()
        super(CapgMyoPiecePlusRawImgMultiStreamData, self).reset()
