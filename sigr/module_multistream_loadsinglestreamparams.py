...
##              pattern='fusion_.*fc\d*_(?:output|weight)',
        ...
        for name in list(self.param):
            for ignore in self.ignore:
                if re.match(ignore, name):
                    ...
                    del self.param[name]
                    break
            if name.startswith('conv') or name.startswith('pixel') or name.startswith('zscore'):
                print name     
#                print self.param[name]
                for i in range(3):                 
                  new_name='stream%d_%s' % (i, name)
                  self.param[new_name] = self.param[name]
                del self.param[name]
                
            if name.startswith('fc') or name.startswith('bottleneck'):
                print name                    
                new_name='fusion_%s' % name
                self.param[new_name] = self.param[name]
                del self.param[name]    
